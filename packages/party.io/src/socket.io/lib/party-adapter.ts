...
class PartyAdapter extends Adapter {
  ...
  async #initialiseConnector(nsp: Namespace, lobby: Party.FetchLobby) {
    ...
    connector.addEventListener("close", () => {
      getLogger("socket.io").debug(
        `[party-adapter] [${this.uid}] connector closed for ${this.partyName}, scheduling reconnect`
      );
  ...
  #scheduleReconnect() {
    ...
    this.#reconnectTimer = setTimeout(() => {
      ...
      this.#initialiseConnector(this.#nsp, this.#lobby).catch((err) => {
        console.error("could not reconnect connector", err);
        this.emitReserved("error", err);
        this.#scheduleReconnect();
      });
    }, 1000);
